...
export default function CreatePostScreen({ navigation }) {
  ...
  const validateAndSubmit = async () => {
    ...
    if (!title.trim() || title.length > 120) {
      Alert.alert(
        language === 'ru' ? 'Ошибка' : language === 'kk' ? 'Қате' : 'Error',
        language === 'ru' ? 'Заголовок обязателен и должен быть ≤ 120 символов' : language === 'kk' ? 'Тақырып міндетті және ≤ 120 таңба болуы керек' : 'Title is required and must be ≤ 120 characters'
      );
      return;
    }
